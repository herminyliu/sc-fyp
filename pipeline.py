...
if __name__ == "__main__":
    ...
    # 聚类分析
    adata = umap_and_marker_gene.cluster_umap(adata, leiden_resolution=0.9, n_neighbors=15, spread=1.0, min_dist=0.5)
    print("=" * 50+"UMAP聚类完成" + "=" * 50)
    # 绘制UMAP图像
    umap_and_marker_gene.plot_umap(adata)
    print("=" * 50+"UMAP图绘制完成"+"=" * 50)
    check_data(adata)
    print(f"细胞被分成了{adata.obs.leiden.unique()}类")

    # 找Marker基因
    umap_and_marker_gene.finding_marker_gene(adata, values_to_plot=None, groupby="cell_type", saving_fig_folder=SAVE_FIG_FOLDER, n_marker_gene=N_MARKER_GENES, method="t-test")
    umap_and_marker_gene.finding_marker_gene(adata, values_to_plot=None, groupby="cell_time", saving_fig_folder=SAVE_FIG_FOLDER, n_marker_gene=N_MARKER_GENES, method="t-test")
    umap_and_marker_gene.finding_marker_gene(adata, values_to_plot=None, groupby="leiden", saving_fig_folder=SAVE_FIG_FOLDER, n_marker_gene=N_MARKER_GENES, method="t-test")
    ...
    umap_and_marker_gene.plot_marker_gene(adata, marker_genes_lst=gene_lst)
    check_data(adata)
