...
def find_transition_genes(
    ...
    if save_csv:
        ...
        print(f"./csv/discrimination_{groupby}_{cell_group_a}_{cell_group_b}_names.csv successful saved.")

    # 提取结果
    result = sc.get.rank_genes_groups_df(adata_sub, group=None)

    # 保存显著基因df
    if save_csv:
        result.to_csv(f"./csv/discrimination_{groupby}_{cell_group_a}_{cell_group_b}_significant_genes.csv")
...
def plot_volcano(result_df,
    ...
    :param logfc_thresh: The minium logfoldchange value to mark as significance, bigger than this value will be filtered.
    :param padj_thresh: The maximum p-value to mark as significance, bigger than this value will be filtered.
    ...
    if marker_list is not None:
        # 移除 marker_list 中不在 result_df["names"] 列中的基因
        original_marker_list = marker_list
        marker_list = [gene for gene in marker_list if gene in result_df["names"].values]

        removed_genes = set(original_marker_list) - set(marker_list)
        if removed_genes:
            print(f"以下基因在 result_df 中不存在，已移除: {removed_genes}")
    ...
    # result_df['pvals_adj']中有部分基因的值非常低，直接被记为0，会导致对数转换错误
    result_df['pvals_adj'] = result_df['pvals_adj'].apply(lambda x: 1 * 10 ** (-300) if x == 0 else x)

    if marker_list is not None:
        result_df['marked'] = "No"
        map_gene_to_row_index: dict = {}

        # -------------------------------------

        marker_list = marker_list + result_df.loc[(result_df['logfoldchanges'] <= -logfc_thresh) & (result_df['pvals_adj'] <= 1e-200), 'names'].to_list()
        marker_list = marker_list + result_df.loc[(result_df['logfoldchanges'] >= logfc_thresh) & (result_df['pvals_adj'] <= 1e-200), 'names'].to_list()
        print(f"补充后的marker list: {marker_list}")

        # -------------------------------------
        for gene in marker_list:
            gene_row_index = result_df.query(f'names == "{gene}"').index  # {gene}必须用双引号括起来！
            result_df.loc[gene_row_index, 'marked'] = "Yes"
            map_gene_to_row_index[gene] = gene_row_index
        # 绘制散点
        # 为什么要弄这么麻烦，用一个for循环？
        # 因为plt.scatter(marker=result_df['marker'].map({'Yes': '*', 'No': 'o'}))不能这样操作
        # marker参数不能接受list类型，虽然逻辑和着色参数c很像，但是只能接受MarkerStyle类型！
        for marker_type, marker_symbol in [('Yes', '*'), ('No', 'o')]:
            subset = result_df[result_df['marked'] == marker_type]
            scatter = plt.scatter(
                x=subset['logfoldchanges'],
                y=-log10(subset['pvals_adj']),
                c=subset['significance'].map({'n.s.': 'grey', 'Up': 'red', 'Down': 'blue'}),
                marker=marker_symbol,  # 每组使用固定标记
                # s=30 if marker_type == 'Yes' else 10,  # 可选：突出显示重点标记
                s=10,
                alpha=0.6,
                label=f'Marker: {marker_type}'
            )

        for gene in marker_list:
            print(result_df.loc[map_gene_to_row_index[gene], 'logfoldchanges'])
            x_target = float(result_df.loc[map_gene_to_row_index[gene], 'logfoldchanges'].iloc[0])
            y_target = float(-log10(result_df.loc[map_gene_to_row_index[gene], 'pvals_adj'].iloc[0]))
            plt.annotate(
                text=gene,  # 标签文本
                xy=(x_target, y_target),  # 点的坐标
                xytext=(x_target + 0.3, y_target + 0.3),  # 标签的位置
                fontsize=10,
                color='#D48713'
            )
    else:
        ...
        scatter = plt.scatter(
            ...
            c=result_df['significance'].map({'n.s.': 'grey', 'Up': 'red', 'Down': 'blue'}),  # 设置区分颜色
            s=10,
            alpha=0.6
        )
...
def cell_group_discrimination(cell_group_a="Primitive Streak", cell_group_b="Nascent mesoderm", groupby="cell_type",
    ...
    if marker_list is None:
        ...
        marker_list = plot_genes_df["names"].to_list()

    # plot_merged_violin_comparison(adata=adata, gene_lst=marker_list,
    ...
    plot_volcano(cell_group_a=cell_group_a, cell_group_b=cell_group_b, result_df=transition_genes_df, save_dir=save_dir,
                 marker_list=marker_list, logfc_thresh=2, padj_thresh=0.001)


def export_diff_genes(f_adata: sc.AnnData, parent_type: str, child1_type: str,
...
if __name__ == "__main__":
    ...
    gene_lst = ["Foxa2", "T", "Mesp1", "Lhx1", "Lhx6", "Gsc", "Hhex", "Cer1", "Phlda2", "Snai1", "Mixl1"]
    ...
    #                           max_padj=1, min_logfc=0,
    #                           groupby="cell_type", save_dir="discrimination_plot", marker_list=gene_lst)
    cell_group_discrimination(cell_group_a="Primitive Streak", cell_group_b="Nascent mesoderm",
                              max_padj=1, min_logfc=0,
                              groupby="cell_type", save_dir="discrimination_plot", marker_list=gene_lst)
    # export_diff_genes(adata, parent_type="Primitive Streak",
    ...
    # plot_merged_violin_comparison(adata=adata, cell_group_a="Primitive Streak", cell_group_b="Anterior Primitive Streak",
    #                               gene_lst=gene_lst, save_dir="temp")
    # plot_merged_violin_comparison(adata=adata, cell_group_a="Primitive Streak", cell_group_b="Nascent mesoderm",
    #                               gene_lst=gene_lst, save_dir="temp")
